...
namespace hmpc::iter
{
    ...
    constexpr decltype(auto) for_each_reverse(hmpc::range<Index, Difference, 0, Count, 1>, F const& f) HMPC_NOEXCEPT
    {
        using index_type = Index;
        return [&f]<index_type... I>(std::integer_sequence<index_type, I...>) -> decltype(auto)
        {
            return (f(hmpc::constant_of<index_type{Count - I - 1}>), ...);
        }(std::make_integer_sequence<index_type, Count>{});
    }

    template<typename Index, typename Difference, Index Start, Index End, typename F>
    constexpr decltype(auto) for_each_reverse(hmpc::range<Index, Difference, Start, End, 1>, F const& f) HMPC_NOEXCEPT
    {
        using index_type = Index;
        ...
        {
            return (f(hmpc::constant_of<index_type{End - I - 1}>), ...);
